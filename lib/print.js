...
let output = `╭──» 𝙀𝙇𝙄𝙏𝙀 𝘽𝙊𝙏 𝙂𝙇𝙊𝘽𝘼𝙇 «───
┊ ${chalk.blue(me + ' ~' + conn.user.name + (conn.user.jid == global.conn.user.jid ? '' : ' (𝗦𝗨𝗕 𝗕𝗢𝗧)'))}
┊ ${chalk.black(chalk.cyan(m.messageStubType ? WAMessageStubType[m.messageStubType] : ''))}
┊ ${chalk.magenta(sender)}
┊ ${chalk.white(m.chat + (chat ? ' ~' + chat : ''))} ${chalk.black(chalk.red(m.mtype ? m.mtype.replace(/message$/i, '').replace('audio', m.msg.ptt ? 'PTT' : 'audio').replace(/^./, v => v.toUpperCase()) : ''))}
╰────────────`

// Filtrar líneas vacías
let lines = output.split('\n')
let filteredLines = lines.filter(line => line.trim() !== '┊' && line.trim() !== '')
