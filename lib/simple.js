...
const proto = baileysModule.proto || baileys.proto
const jidDecodeFn =
typeof baileys.jidDecode === 'function' ? baileys.jidDecode :
typeof baileysModule.jidDecode === 'function' ? baileysModule.jidDecode :
undefined
const _makeWaSocket =
typeof baileysModule.default === 'function' ? baileysModule.default :
typeof baileys.makeWASocket === 'function' ? baileys.makeWASocket :
typeof baileysModule.makeWASocket === 'function' ? baileysModule.makeWASocket :
typeof baileys.default === 'function' ? baileys.default :
undefined
const makeWALegacySocketFactory =
typeof baileys.makeWALegacySocket === 'function' ? baileys.makeWALegacySocket :
typeof baileysModule.makeWALegacySocket === 'function' ? baileysModule.makeWALegacySocket :
undefined
...
/**
   ...
   */
const socketFactory = global.opts['legacy'] ? makeWALegacySocketFactory : _makeWaSocket
if (typeof socketFactory !== 'function') {
throw new TypeError(`Baileys socket factory not found (legacy=${Boolean(global.opts['legacy'])})`)
}
...
const WebMessageInfo = getWebMessageInfo()
if (!WebMessageInfo?.fromObject) return copy
return WebMessageInfo.fromObject(copy)
},
enumerable: true
},
copyNForward: {
...
const WebMessageInfo = getWebMessageInfo()
if (!WebMessageInfo?.prototype) {
console.warn('[simple.js] proto.WebMessageInfo no disponible; se omite parche de serializacion')
return {}
}
...
return WebMessageInfo.fromObject({
key: {
fromMe: this.fromMe,
remoteJid: this.chat,
id: this.id
...
/**
             ...
             */
value() {
const M = getWebMessageInfo()
if (!M?.fromObject || !M?.toObject) return this.vM
...
value() {
const M = getWebMessageInfo()
if (!M?.fromObject || !M?.toObject) return this
...
const WebMessageInfo = getWebMessageInfo()
if (!WebMessageInfo?.fromObject) return smsg(this.conn, this.conn?.loadMessage(this.quoted.id) || this.quoted.vM)
const q = WebMessageInfo.fromObject(this.conn?.loadMessage(this.quoted.id) || this.quoted.vM)
...
if (/:\d+@/gi.test(this)) {
if (typeof jidDecodeFn !== 'function') return this.trim()
const decode = jidDecodeFn(this) || {}
