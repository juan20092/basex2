...
// ... (el resto del objeto user se mantiene igual)
// Para ahorrar espacio, mantengo la estructura pero elimino el contenido repetido
}

// ... (el resto del código de inicialización de la base de datos se mantiene igual)
...
// 🔥🔥🔥 CORRECCIÓN DEFINITIVA PARA DETECCIÓN DE ADMINISTRADORES 🔥🔥🔥
let isAdmin = false
let isBotAdmin = false 
let isRAdmin = false

if (m.isGroup && participants && participants.length > 0) {
    // Búsqueda directa y robusta
    for (let participant of participants) {
        const participantJid = participant.id || participant.jid
        
        // Verificar si es el usuario que envió el mensaje
        if (areJidsSameUser(participantJid, m.sender)) {
            isAdmin = participant.admin === 'admin' || 
                     participant.admin === 'superadmin' || 
                     participant.admin === true
            isRAdmin = participant.admin === 'superadmin'
        }
        
        // Verificar si es el bot
        if (areJidsSameUser(participantJid, this.user.jid)) {
            isBotAdmin = participant.admin === 'admin' || 
                        participant.admin === 'superadmin' || 
                        participant.admin === true
        }
...
if (m.isGroup && m.text === '.debug') {
    ...
    let debugInfo = `🔍 *DEBUG ADMIN INFO*\\n`
    debugInfo += `👤 *Tu número:* ${m.sender}\\n`
    debugInfo += `🤖 *Bot número:* ${this.user.jid}\\n`
    debugInfo += `📊 *Total participantes:* ${participants.length}\\n\\n`
    debugInfo += `👑 *RESULTADOS:*\\n`
    debugInfo += `• isAdmin: ${isAdmin ? '✅' : '❌'}\\n`
    debugInfo += `• isBotAdmin: ${isBotAdmin ? '✅' : '❌'}\\n`
    debugInfo += `• isRAdmin: ${isRAdmin ? '✅' : '❌'}\\n\\n`
    debugInfo += `⚠️ *NOTA:* El bot debe ser admin para detectar otros admins`
    
    await this.sendMessage(m.chat, { text: debugInfo }, { quoted: m })
    return
}
*/
...
if (command === 'debugadmin') {
    ...
    for (let p of participants) {
        ...
        if (areJidsSameUser(pJid, m.sender)) {
            debugInfo += `👑 *TU INFORMACIÓN:*\\n`
            debugInfo += `• Admin: ${p.admin || 'NO'}\\n`
            debugInfo += `• isAdmin: ${isAdmin ? '✅' : '❌'}\\n`
            debugInfo += `• isRAdmin: ${isRAdmin ? '✅' : '❌'}\\n\\n`
            userFound = true
        }
        
        if (areJidsSameUser(pJid, this.user.jid)) {
        ...
        if (userFound && botFound) break
    }
    
    if (!userFound) {
        debugInfo += `❌ *NO SE ENCONTRÓ tu información en los participantes*\\n\\n`
    }
    if (!botFound) {
        debugInfo += `❌ *NO SE ENCONTRÓ la información del bot en los participantes*\\n\\n`
    }
    
    debugInfo += `⚠️ *SOLUCIÓN:*\\n`
    debugInfo += `• El bot DEBE ser administrador del grupo\\n`
    debugInfo += `• Tu número DEBE estar como administrador\\n`
    debugInfo += `• Reinicia el bot si hiciste cambios recientes`
...
if (db.data.chats[m.chat].reaction && m.text.match(/(bufy)/gi)) {
let emot = pickRandom([
'😀', '😃', '😄', '😁', '😆', '🥹', '😅', '😂', '🤣', '🥲', '☺️', '😊', '😇', '🙂', '🙃', '😉', '😌', '😍', '🥰', '😘', '😗', '😙', '😚', '😋', '😛', '😝', '😜', '🤪', '🤨', '🧐', '🤓', '😎', '🥸', '🤩', '🥳', '😏', '😒', '😞', '😔', '😟', '😕', '🙁', '☹️', '😣', '😖', '😫', '😩', '🥺', '😢', '😭', '😤', '😠', '😡', '🤬', '🤯', '😳', '🥵', '🥶', '😶‍🌫️', '😱', '😨', '😰', '😥', '😓', '🤗', '🤔', '🫣', '🤭', '🫢', '🫡', '🤫', '🫠', '🤥', '😶', '🫥', '😐', '🫤', '😑', '🫨', '😬', '🙄', '😯', '😦', '😧', '😮', '😲', '🥱', '😴', '🤤', '😪', '😮‍💨', '😵', '😵‍💫', '🤐', '🥴', '🤢', '🤮', '🤧', '😷', '🤒', '🤕', '🤑', '🤠', '😈', '👿', '👺', '🤡', '💩', '👻', '😺', '😸', '😹', '😻', '😼', '😽', '🙀', '😿', '😾', '🫶', '👍', '✌️', '🙏', '🫵', '🤏', '🤌', '☝️', '🖕', '🙏', '🫵', '🫂', '🐱', '🤹‍♀️', '🤹‍♂️', '🗿', '✨', '⚡', '🔥', '🌈', '🩷', '❤️', '🧡', '💛', '💚', '🩵', '💙', '💜', '🐣', '🐤', '🐈'
])
...
// ... (el resto de las funciones exportadas se mantienen igual)
                ...
                for (let user of participants) {
                    ...
                    try {
                        ...
                        if (chat.antifake && isBotAdminNn && action === 'add') {
                            const prefijosPredeterminados = [ 2, 4, 6, 7, 234, 9]
