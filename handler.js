...
// 🔥 CORRECCIÓN MEJORADA PARA DETECCIÓN DE ADMINISTRADORES
const getGroupAdmins = (participants) => {
    const admins = []
    for (const p of participants) {
        const jid = p.id || p.jid
        if (p.admin === 'admin' || p.admin === 'superadmin' || p.admin === true) {
            admins.push(jidNormalizedUser(jid))
        }
    }
    return admins
}
...
const isAdmin = groupAdmins.includes(jidNormalizedUser(m.sender))
const isBotAdmin = groupAdmins.includes(jidNormalizedUser(this.user.jid))
const isRAdmin = m.isGroup ? participants.some(p => 
    areJidsSameUser(p.id || p.jid, m.sender) && p.admin === 'superadmin') : false
